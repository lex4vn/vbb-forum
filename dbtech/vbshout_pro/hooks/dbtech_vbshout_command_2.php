...
switch ($matches[1])
{
	...
	case '/unsilence':
		$handled = true;	
		if (!VBSHOUT::$instance['permissions_parsed']['cansilence'])
		{
		...
		VBSHOUT::log_command(($matches[1] == '/silence' ? 'silence' : 'unsilence'), $exists['userid']);		
		
		if (VBSHOUT::$instance['permissions_parsed']['showaction'])
		{
			// Print success message
			$this->set('type', 		VBSHOUT::$shouttypes['system']);
			$this->set('message', 	($matches[1] == '/silence' ? construct_phrase($vbphrase['dbtech_vbshout_silenced_successfully'], $matches[2]) : construct_phrase($vbphrase['dbtech_vbshout_unsilenced_successfully'], $matches[2])));
			$return_value = true;
		}
		else
		{
	...
	case '/prune':
		$handled = true;	
		if (!VBSHOUT::$instance['permissions_parsed']['canprune'])
		{
		...
		VBSHOUT::log_command('pruneuser', $exists['userid']);
