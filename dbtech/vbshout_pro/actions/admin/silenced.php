...
// #############################################################################
if ($_REQUEST['action'] == 'silenced' OR empty($_REQUEST['action']))
{
	...
	if ($numusers = $db->num_rows($users))
	{
		...
		print_form_header('vbshout', 'silenced');
		construct_hidden_code('action', 'manage');
...
if ($_REQUEST['action'] == 'manage')
{
	...
	define('CP_REDIRECT', 'vbshout.php?do=silenced');
