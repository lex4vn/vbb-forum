...
if ($chatroom['membergroupids'])
{
	...
	if (!is_member_of($vbulletin->userinfo, $membergroupids) AND !$vbshout->permissions['canmodchat'])
	{
		// Gtfo.
	...
	// Query active users
	$SQL = "
		SELECT COUNT(user.userid) AS totalentries
...
else
{
	...
	if (!in_array($chatroomid, $memberof) AND !$vbshout->permissions['canmodchat'])
	{
		// Gtfo.
		eval(standard_error(fetch_error('dbtech_vbshout_cannot_access_chatroom')));
	}	
	...
	// Query active users
	$SQL = "
		SELECT COUNT(userid) AS totalentries
...
while ($activeusers_r = $db->fetch_array($activeusers_q))
{
	...
	if (!$chatroom['membergroupids'])
	{
		$creator = ($chatroom['creator'] == $vbulletin->userinfo['userid'] OR $vbshout->permissions['canmodchat'] ? true : false);
	}
