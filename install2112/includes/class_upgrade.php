...
abstract class vB_Upgrade_Abstract
{
	...
	*/
	protected function load_script($version)
	{
		...
		{
			$this->startup_errors[] = sprintf($this->phrase['core']['invalid_version_x'], htmlspecialchars_uni($version));
			return false;
		}
	...
	protected function get_upgrade_start($version = null)
	{
			...
			if ($log['step'] == 0)
			{
				...
				*/
				...
				else
				{
					...
					{
						if (($currentkey = array_search($scriptver, $this->endscripts)) !== false)
						{
							$scriptinfo['version'] = $this->endscripts[$currentkey + 1];
						}
						else
						{
							$scriptinfo['version'] = $this->endscripts[0];
						}
					}
					else
					{
						$scriptinfo['version'] = 'final';	// process XML and Products
					}
				}
		...
		else
		{
			...
			*/
			...
			else if (($version == 'install' OR ($versionkey == count($shortversions))) AND $this->install_suite())
			{
				$scriptinfo['version'] = 'vbblog';
			}
			else if ($shortver == 'vbblog' AND $this->install_suite())
			{
				$scriptinfo['version'] = 'vbcms';
			}
			else if (in_array(intval($this->registry->versionnumber), array(3,4)))
			{
	...
	*/
	protected function sync_database()
	{
		...
		$this->db->query_write("ALTER TABLE " . TABLE_PREFIX . "post_parsed RENAME " . TABLE_PREFIX . "postparsed");

		// These tables are referenced by upgrade scripts that predate these modifications
		$this->db->query_write("ALTER TABLE " . TABLE_PREFIX . "upgradelog ADD only TINYINT NOT NULL DEFAULT '0'");
		$this->db->query_write("ALTER TABLE " . TABLE_PREFIX . "adminmessage ADD args MEDIUMTEXT");
		$this->db->show_errors();
	}
...
abstract class vB_Upgrade_Version
{
	...
	public function __construct(&$registry, $phrase, $maxversion)
	{
		if (is_object($registry))
		{
			...
			if (VB_AREA == 'Upgrade')
			{
				...
				{
					$this->set_option('upgrade_from', 'version', $this->registry->options['templateversion'], 'string', '' );
	...
	*/
	...
	protected function set_option($varname, $grouptitle, $value, $datatype, $default_value = false )
	{
		...
		if (!$row or !$row['value'])
		{
			$sql = "INSERT INTO " . TABLE_PREFIX . "setting
					(varname, grouptitle, value, datatype" . ($default_value ? ',default_value' : '') .
		...
		if ($complete)
		{
			...
			if ($this->SHORT_VERSION == 'final' OR $only)
			{
				...
				$this->db->query_write("
					...
					WHERE script IN ('final', 'vbblog', 'vbcms')
				");
