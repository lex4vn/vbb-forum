...
function
fr_update_push_user ($username, $fr_b = false, $token = '')
{
    global $vbulletin;
    ...
    if ($tableinfo && $vbulletin->userinfo['userid']) {
        if ($username || $token) {
            ...
            // See if we already have an entry for this vb_userid
		...
		SELECT id FROM " . TABLE_PREFIX . "forumrunner_push_users
            ...
            if ($fr_user) {
                $vbulletin->db->query_write("
                    ...
                    SET vb_userid = {$vbulletin->userinfo['userid']}, last_login = NOW(), b = " . ($fr_b ? 1 : 0) . ", token = '" . $vbulletin->db->escape_string($token) . "', fr_username = '" . $vbulletin->db->escape_string($username) . "'
                    WHERE id = {$fr_user['id']}
                ");
            } else {
                $vbulletin->db->query_write("
                    INSERT INTO " . TABLE_PREFIX . "forumrunner_push_users
                    (vb_userid, fr_username, b, last_login, token)
                    VALUES ({$vbulletin->userinfo['userid']}, '" . $vbulletin->db->escape_string($username) . "', " . ($fr_b ? 1 : 0) . ", NOW(), '" . $vbulletin->db->escape_string($token) . "')
                ");
            }
        } else {
