...
if ($_POST['do'] == 'postreply')
{
	...
	else
	{
		...
		// Expire the CMS cache entries for this comments thread.
		if ($vbulletin->GPC_exists['return_node'] AND $vbulletin->GPC['return_node'])
		{
			...
			$expire_cache[] = 'cms_comments_thread_' . $threadinfo['threadid'];
		...
		if ($vbulletin->GPC['ajax'])
		{
			...
			{
				// This is a CMS comment, so we dont need most of the stuff below, its just not used.
				// All we need is to do the mark read & facebook stuff, and then return some valid XML.
				
				// Still run the hook, just in case someone wants to do something funky.
